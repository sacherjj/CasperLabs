...
def test_persistent_dag_store(command_line_options_fixture, docker_client_fixture):
    with conftest.testing_context(
        ...
        with complete_network(context) as network:
            ...
            wait_for_node_started(network.peers[0], context.node_startup_timeout * 3)
            wait_for_requested_for_fork_tip(network.peers[0], context.node_startup_timeout * 3)
            wait_for_connected_to_node(network.bootstrap, network.peers[0].name, context.node_startup_timeout)
            wait_a_minute()
            ...
            wait_for_finalised_hash(network.bootstrap, hash_string, context.node_startup_timeout)
            wait_for_finalised_hash(network.peers[0], '', context.node_startup_timeout)
