...
pub extern "C" fn call() {
    ...
    // For now, we are storing validators in `known_keys` map of the PoS contract
    // in the form: key: "v_{validator_pk}_{validator_stake}", value: doesn't
    // matter.
    let mut known_keys: BTreeMap<String, Key> = genesis_validators
        .iter()
        .map(|(pub_key, balance)| {
    ...
    // Include the mint contract in its known_keys
    known_keys.insert(String::from(MINT_NAME), Key::URef(mint_uref));
    ...
    // Include PoS purses in its known_keys
    [
    ...
    .iter()
    .for_each(|(name, uref)| {
        known_keys.insert(String::from(*name), Key::URef(*uref));
    });

    let contract = contract_api::fn_by_name("pos_ext", known_keys);
