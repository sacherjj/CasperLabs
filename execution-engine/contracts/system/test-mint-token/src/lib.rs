...
pub extern "C" fn call() {
    ...
    let purse1: Key =
        contract_api::runtime::call_contract(mint.clone(), &("create", amount1), &vec![]);

    let amount2 = U512::from(300);
    let purse2: Key =
        contract_api::runtime::call_contract(mint.clone(), &("create", amount2), &vec![]);

    let result: String = contract_api::runtime::call_contract(
        mint.clone(),
    ...
    let new_amount1: Option<U512> =
        contract_api::runtime::call_contract(mint.clone(), &("balance", purse1), &vec![purse1]);
    let new_amount2: Option<U512> =
        contract_api::runtime::call_contract(mint.clone(), &("balance", purse2), &vec![purse2]);
