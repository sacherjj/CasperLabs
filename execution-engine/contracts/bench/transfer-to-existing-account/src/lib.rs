...
pub extern "C" fn call() {
    let public_key: PublicKey = contract_api::runtime::get_arg(Arg::PublicKey as u32)
        .unwrap_or_revert_with(ApiError::MissingArgument)
        .unwrap_or_revert_with(ApiError::InvalidArgument);
    let amount: U512 = contract_api::runtime::get_arg(Arg::Amount as u32)
        .unwrap_or_revert_with(ApiError::MissingArgument)
        .unwrap_or_revert_with(ApiError::InvalidArgument);
    let result = contract_ffi::contract_api::system::transfer_to_account(public_key, amount)
        .unwrap_or_revert();
    match result {
        ...
        TransferredTo::NewAccount => {
            contract_api::runtime::revert(ApiError::User(Error::TransferredToNewAccount as u16))
        }
    }
}
