...
pub extern "C" fn call() {
    let turef = match contract_api::runtime::get_arg(Args::DoNothingURef as u32) {
        Some(Ok(data)) => TURef::from_uref(data).unwrap_or_else(|_| {
            contract_api::runtime::revert(Error::User(CustomError::InvalidTURef as u16))
        }),
        Some(Err(_)) => {
            contract_api::runtime::revert(Error::User(CustomError::InvalidDoNothingURefArg as u16))
        }
        None => {
            contract_api::runtime::revert(Error::User(CustomError::MissingDoNothingURefArg as u16))
        }
    };
