...
pub extern "C" fn call() {
    match contract_api::account::add_associated_key(PublicKey::new([123; 32]), Weight::new(100)) {
        Err(AddKeyFailure::DuplicateKey) => {}
        Err(_) => contract_api::runtime::revert(Error::User(50)),
        Ok(_) => {}
    };
    ...
    let deploy_threshold: Weight = contract_api::runtime::get_arg(1)
        .unwrap_or_revert_with(Error::MissingArgument)
        .unwrap_or_revert_with(Error::InvalidArgument);
    ...
    if deploy_threshold != Weight::new(0) {
        contract_api::account::set_action_threshold(ActionType::Deployment, deploy_threshold)
