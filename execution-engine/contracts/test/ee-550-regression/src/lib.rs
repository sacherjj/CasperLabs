...
pub extern "C" fn call() {
    ...
    match pass.as_str() {
        "init_remove" => {
            account::add_associated_key(PublicKey::from_ed25519_bytes(KEY_1_ADDR), Weight::new(2))
                .unwrap_or_revert_with(Error::AddKey1);
            account::add_associated_key(
                PublicKey::from_ed25519_bytes(KEY_2_ADDR),
                Weight::new(255),
            )
        ...
        "test_remove" => {
            ...
            // threshold
            account::remove_associated_key(PublicKey::from_ed25519_bytes(KEY_1_ADDR))
                .unwrap_or_revert_with(Error::RemoveKey);
        }

        "init_update" => {
            account::add_associated_key(PublicKey::from_ed25519_bytes(KEY_1_ADDR), Weight::new(3))
                .unwrap_or_revert_with(Error::AddKey1);
            account::add_associated_key(
                PublicKey::from_ed25519_bytes(KEY_2_ADDR),
                Weight::new(255),
        ...
        "test_update" => {
            ...
            account::update_associated_key(
                PublicKey::from_ed25519_bytes(KEY_1_ADDR),
                Weight::new(1),
            )
