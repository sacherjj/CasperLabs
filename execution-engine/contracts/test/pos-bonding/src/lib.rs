...
pub extern "C" fn call() {
    ...
    if command == TEST_BOND {
        ...
        let account: PublicKey = contract_api::runtime::get_arg(1)
            .unwrap_or_revert_with(ApiError::MissingArgument)
            .unwrap_or_revert_with(ApiError::InvalidArgument);
        ...
        let maybe_amount: Option<U512> = contract_api::runtime::get_arg(1)
            .unwrap_or_revert_with(ApiError::MissingArgument)
            .unwrap_or_revert_with(ApiError::InvalidArgument);
        unbond(&pos_pointer, maybe_amount);
    } else {
        contract_api::runtime::revert(ApiError::User(Error::UnknownCommand as u16));
    }
}
