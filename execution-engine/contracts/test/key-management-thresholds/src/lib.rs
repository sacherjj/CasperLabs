...
pub extern "C" fn call() {
    ...
    if stage == "init" {
        ...
        account::add_associated_key(PublicKey::from_ed25519_bytes([42; 32]), Weight::new(100))
            .unwrap_or_revert();
        // this key will be used to test permission denied when removing keys with low
        // total weight
        account::add_associated_key(PublicKey::from_ed25519_bytes([43; 32]), Weight::new(1))
            .unwrap_or_revert();
        account::add_associated_key(PublicKey::from_ed25519_bytes([1; 32]), Weight::new(1))
            .unwrap_or_revert();
        ...
        match account::add_associated_key(PublicKey::from_ed25519_bytes([44; 32]), Weight::new(1)) {
            Ok(_) => runtime::revert(ApiError::User(200)),
            Err(AddKeyFailure::PermissionDenied) => {}
            Err(_) => runtime::revert(ApiError::User(201)),
        }

        match account::update_associated_key(
        ...
        match account::remove_associated_key(PublicKey::from_ed25519_bytes([43; 32])) {
        ...
        account::add_associated_key(PublicKey::from_ed25519_bytes([44; 32]), Weight::new(1))
            .unwrap_or_revert();
        // Updates [43;32] key weight created in init stage
        account::update_associated_key(PublicKey::from_ed25519_bytes([44; 32]), Weight::new(2))
            .unwrap_or_revert();
        // Removes [43;32] key created in init stage
        account::remove_associated_key(PublicKey::from_ed25519_bytes([44; 32])).unwrap_or_revert();
