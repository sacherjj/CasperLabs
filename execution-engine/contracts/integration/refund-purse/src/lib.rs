...
pub extern "C" fn call() {
    ...
    let refund_purse = match get_refund_purse(&pos_pointer) {
        None => contract_api::runtime::revert(Error::User(2)),
        Some(x) if x.value().addr() == p1.value().addr() => x.value(),
        Some(_) => contract_api::runtime::revert(Error::User(3)),
    };
    ...
    match get_refund_purse(&pos_pointer) {
        None => contract_api::runtime::revert(Error::User(5)),
        Some(x) if x.value().addr() == p2.value().addr() => (),
        Some(_) => contract_api::runtime::revert(Error::User(6)),
    }
}
