...
pub extern "C" fn call() {
    let hello_name_uref =
        contract_api::runtime::get_key("hello_name").unwrap_or_revert_with(Error::User(100));
    ...
    let arg = "World";
    let result: String = contract_api::runtime::call_contract(pointer, &(arg,), &Vec::new());
    assert_eq!("Hello, World", result);

    //store the result at a uref so it can be seen as an effect on the global state
    contract_api::runtime::put_key(
        "helloworld",
        &contract_api::storage::new_turef(Value::String(result)).into(),
    );
}
