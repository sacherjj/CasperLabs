...
pub extern "C" fn hello_name_ext() {
    let name: String = contract_api::runtime::get_arg(0)
        .unwrap_or_revert_with(Error::MissingArgument)
        .unwrap_or_revert_with(Error::InvalidArgument);
    let y = hello_name(&name);
    contract_api::runtime::ret(&y, &Vec::new());
}
...
fn sub(name: String) -> Option<TURef<Vec<String>>> {
    if contract_api::runtime::has_key(&name) {
        ...
        Some(contract_api::storage::new_turef(init_message)) //already subscribed
    } else {
        let init_message = vec![String::from("Welcome!")];
        let new_turef = contract_api::storage::new_turef(init_message);
        contract_api::runtime::put_key(&name, &new_turef.clone().into());
        update_list(name);
        Some(new_turef)
    }
}
...
pub extern "C" fn mailing_list_ext() {
    ...
    match method_name.as_str() {
        "sub" => match sub(contract_api::runtime::get_arg(1)
            .unwrap_or_revert_with(Error::MissingArgument)
            .unwrap_or_revert_with(Error::InvalidArgument))
        {
            Some(turef) => {
        ...
        "pub" => {
            publish(
                contract_api::runtime::get_arg(1)
                    .unwrap_or_revert_with(Error::MissingArgument)
                    .unwrap_or_revert_with(Error::InvalidArgument),
            );
        }
...
pub extern "C" fn call() {
    // hello_name
    let pointer = contract_api::storage::store_function_at_hash("hello_name_ext", BTreeMap::new());
    contract_api::runtime::put_key("hello_name", &pointer.into());

    // counter
    let counter_local_turef = contract_api::storage::new_turef(0); //initialize counter
    ...
    let _counter_hash = contract_api::storage::store_function_at_hash("counter_ext", counter_urefs);
    contract_api::runtime::put_key("counter", &_counter_hash.into());

    // mailing list
    let init_list: Vec<String> = Vec::new();
    let list_turef = contract_api::storage::new_turef(init_list);
