...
mod tests {
    ...
    fn create_test_pairs() -> [TestPair; 2] {
        [
            TestPair {
                key: Key::Account(PublicKey::from_ed25519_bytes([1_u8; 32])),
                value: StoredValue::CLValue(CLValue::from_t(1_i32).unwrap()),
            },
            TestPair {
                key: Key::Account(PublicKey::from_ed25519_bytes([2_u8; 32])),
    ...
    fn create_test_pairs_updated() -> [TestPair; 3] {
        [
            TestPair {
                key: Key::Account(PublicKey::from_ed25519_bytes([1u8; 32])),
                value: StoredValue::CLValue(CLValue::from_t("one".to_string()).unwrap()),
            },
            TestPair {
                key: Key::Account(PublicKey::from_ed25519_bytes([2u8; 32])),
                value: StoredValue::CLValue(CLValue::from_t("two".to_string()).unwrap()),
            },
            TestPair {
                key: Key::Account(PublicKey::from_ed25519_bytes([3u8; 32])),
