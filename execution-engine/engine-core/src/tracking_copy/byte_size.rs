...
impl ByteSize for Account {
    fn byte_size(&self) -> usize {
        std::mem::size_of::<Self>() + self.heap_size() + self.known_keys().byte_size()
    }
}
...
impl HeapSizeOf for Account {
    fn heap_size(&self) -> usize {
        self.known_keys().heap_size()
    }
}

impl HeapSizeOf for Contract {
    fn heap_size(&self) -> usize {
        self.known_keys().heap_size() + self.bytes().len()
    }
}
