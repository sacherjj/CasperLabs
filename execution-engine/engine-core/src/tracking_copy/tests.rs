...
proptest! {
    ...
    fn query_contract_state(
        ...
        known_keys.insert(name.clone(), k);
        let contract: Value = Contract::new(body, known_keys, ProtocolVersion::new(1)).into();
    ...
    fn query_account_state(
        ...
        let correlation_id = CorrelationId::new();
        let known_keys = iter::once((name.clone(), k)).collect();
    ...
    fn query_path(
        ...
        let mut contract_known_keys = BTreeMap::new();
        contract_known_keys.insert(state_name.clone(), k);
        let contract: Value = Contract::new(body, contract_known_keys, ProtocolVersion::new(1)).into();
        let contract_key = Key::Hash(hash);

        // create account which knows about contract
        let mut account_known_keys = BTreeMap::new();
        account_known_keys.insert(contract_name.clone(), contract_key);
        let purse_id = PurseId::new(URef::new([0u8; 32], AccessRights::READ_ADD_WRITE));
        let associated_keys = AssociatedKeys::new(PublicKey::new(pk), Weight::new(1));
        let account = Account::new(
            pk,
