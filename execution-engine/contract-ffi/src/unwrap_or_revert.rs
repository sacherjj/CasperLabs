...
/// `contract_api::runtime::revert()` if this fails.  It is implemented for `Result` and `Option`.
pub trait UnwrapOrRevert<T> {
    /// Unwraps the value into its inner type or calls `contract_api::runtime::revert()` with a
    /// predetermined error code on failure.
    fn unwrap_or_revert(self) -> T;

    /// Unwraps the value into its inner type or calls `contract_api::runtime::revert()` with the
...
impl<T, E: Into<Error>> UnwrapOrRevert<T> for Result<T, E> {
    fn unwrap_or_revert(self) -> T {
        self.unwrap_or_else(|error| contract_api::runtime::revert(error.into()))
    }
...
impl<T> UnwrapOrRevert<T> for Result<T, bytesrepr::Error> {
    fn unwrap_or_revert(self) -> T {
        self.unwrap_or_else(|_| contract_api::runtime::revert(Error::Deserialize))
    }
...
impl<T> UnwrapOrRevert<T> for Option<T> {
    fn unwrap_or_revert(self) -> T {
        self.unwrap_or_else(|| contract_api::runtime::revert(Error::None))
    }

    fn unwrap_or_revert_with<E: Into<Error>>(self, error: E) -> T {
        self.unwrap_or_else(|| contract_api::runtime::revert(error.into()))
    }
}
