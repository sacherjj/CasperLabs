...
fn should_maintain_local_state_across_upgrade() {
    ...
    // upgrade local_state_stored contract
    {
        let exec_request = {
            ...
            ExecuteRequestBuilder::standard(
                ...
                &contract_name,
                runtime_args! {
                    "contract_package" => stored_package_hash,
                },
            )
            .build()
        };
    ...
    assert!(
        ...
        "local state should include elements from the original version and the upgraded version: {}", write
    );
}
