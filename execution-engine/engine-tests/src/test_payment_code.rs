...
fn should_raise_insufficient_payment_when_caller_lacks_minimum_balance() {
    ...
    let error_message = {
        let execution_result = crate::test_support::get_success_result(&account_1_response);
...
fn should_raise_insufficient_payment_when_payment_code_does_not_pay_enough() {
    ...
    let execution_result = crate::test_support::get_success_result(&response);
    let error_message = crate::test_support::get_error_message(execution_result);
...
fn should_correctly_charge_when_session_code_runs_out_of_gas() {
    ...
    let motes = crate::test_support::get_success_result(&response).cost * CONV_RATE;
