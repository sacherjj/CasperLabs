...
use crate::test_support::{WasmTestBuilder, DEFAULT_BLOCK_TIME};
...
fn should_manage_associated_key() {
    ...
    let account_1: Account = {
        ...
        crate::test_support::get_account(&transforms[1], &account_key).expect("should get account")
    };
    ...
    let account_1: Account = {
        let tmp = builder.clone();
        let transforms = tmp.get_transforms();
        crate::test_support::get_account(&transforms[2], &account_key).expect("should get account")
    };
