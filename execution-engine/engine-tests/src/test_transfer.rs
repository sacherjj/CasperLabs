...
use crate::test_support::{self, DEFAULT_BLOCK_TIME};
...
fn should_transfer_to_account() {
    ...
    let genesis_transforms = crate::test_support::get_genesis_transforms(&genesis_response);

    let mint_contract_uref =
        crate::test_support::get_mint_contract_uref(&genesis_transforms, &contracts)
            .expect("should get uref");

    let mut test_context = TestContext::new(mint_contract_uref);

    let genesis_account =
        crate::test_support::get_account(&genesis_transforms, &genesis_account_key)
...
fn should_transfer_from_account_to_account() {
    ...
    let commit_request =
        crate::test_support::create_commit_request(genesis_hash, &exec_1_transforms);
...
fn should_fail_when_insufficient_funds() {
    ...
    let commit_request =
        crate::test_support::create_commit_request(commit_hash, &exec_2_transforms);
...
fn should_transfer_total_amount() {
    let mut builder = crate::test_support::WasmTestBuilder::default();
