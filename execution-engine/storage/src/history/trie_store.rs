...
/// Represents a transaction which can be committed or aborted.
pub trait Transaction: Sized {
    /// Represents an error which can occur while reading or writing during a
    /// transaction, or committing the transaction.
    type Error;

    /// Represents an entity which is being read from or written to during a
    /// transaction.
    type Handle;
...
/// Represents a transaction with the capability to read from a given [`Handle`](Transaction::Handle).
...
/// Represents a transaction with the capability to write to a given [`Handle`](Transaction::Handle).
...
/// Represents a source of transactions e.g. values that implement [`Readable`]
/// and/or [`Writable`].
pub trait TransactionSource<'a> {
    /// Represents an error which can occur while creating a read or read-write
    /// transaction.
    type Error;
...
/// Represents an entity which persists [`Trie`] values at their hashes.
pub trait TrieStore<K, V> {
    /// Represents an error which can occur while getting a value out of or
    /// putting a value into a trie store.
    type Error;
