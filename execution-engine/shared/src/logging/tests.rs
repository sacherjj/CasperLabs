...
use crate::logging::logger::{set_buffered_logger, LogBufferProvider};
...
fn log_duration_should_log_correlation_id() {
    ...
    let log_items = logger::BUFFERED_LOGGER
        .take_correlated(&correlation_id.to_string())
...
#[test]
fn capture_duration_should_return_value() {
    setup();

    let correlation_id = CorrelationId::new();

    const METRIC: &str = "duration_seconds";
    const TAG: &str = "test_tag";

    assert!(
        capture_duration!(correlation_id, METRIC, TAG, true),
        "should have returned value"
    )
}

#[test]
fn capture_elapsed_should_return_elapsed() {
    setup();
    ...
    let elapsed = capture_elapsed!(correlation_id, "should_capture_elapsed", "test", start);

    assert_ne!(elapsed, None, "should have a duration");

    let stop_watch = start.elapsed().unwrap();

    let duration = elapsed.unwrap();

    assert!(
        duration < stop_watch,
        "elapsed should be less than stop_watch"
    );
    ...
    assert!(
        ns > 0,
        format!("elapsed should be greater than 0ns; was: {}ns", ns)
    );
}
