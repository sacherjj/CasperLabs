...
object scalatestcontrib extends Matchers with Assertions {
  ...
  /** If a feature is missing we can do `Log[F].debug("FIXME: Implement feature X!")`
	  ...
	  * An alternative is to call `pendingUntilFixed` which will turn pending while the test
	  * fails and fail_the test when every other assertion passes, until the mark is removed.
