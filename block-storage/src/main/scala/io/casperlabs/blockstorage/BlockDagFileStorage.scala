...
final class BlockDagFileStorage[F[_]: Concurrent: Sync: Log: BlockStore] private (
  ...
  private def squashLatestMessagesDataFile(): F[Unit] =
    for {
      ...
      tmpSquashedData <- createTmpFile(
                          "casperlabs-block-dag-store-latest-messages-",
                          "-squashed-data"
                        )
      tmpSquashedCrc <- createTmpFile(
                         "casperlabs-block-dag-store-latest-messages-",
                         "-squashed-crc"
                       )
...
object BlockDagFileStorage {
  ...
  def createEmptyFromGenesis[F[_]: Monad: Concurrent: Sync: Log](
    ...
    for {
      lock                  <- Semaphore[F](1)
      _                     = Files.createFile(config.latestMessagesLogPath)
      ...
      crcBytes <- crc.bytes
      _        = Files.write(config.latestMessagesLogPath, data)
