...
object EventStream {
  def create[F[_]: Concurrent: FinalizedBlocksStream](scheduler: Scheduler): EventStream[F] = {
    val source =
      ConcurrentSubject.publish[Event](OverflowStrategy.DropOld(bufferSize = 10))(scheduler)
    new EventStream[F] {
