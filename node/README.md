...
Run:
```
sudo sbt -Dsbt.log.noformat=true clean node/docker:publishLocal
```
...
To see the list of available flags you can run `./node --help`
...
Node needs to have read and write access to a folder called data directory. By default that folder is `$HOME/.casperlabs`. User can control that value by providing `--data-dir` flag. 
...
You can also [build a docker image yourself](#building-via-docker) and then run it.  The Docker image requires the folder `var/lib/node` to be present and accessible at runtime.  

```
$ docker run -ti -v "$HOME/node":/var/lib/node io.casperlabs/node run 
...
17:12:22.963 [main] INFO main - Listening for traffic on casperlabs://6403d6e9f0874d31940a654f3f52a830@192.168.1.123:40400.
...
```
$ docker run -ti -p 12345:12345 io.casperlabs/node:latest run -p 12345 --host 1.2.3.4
```
...
11:21:01.121 [main] INFO  main - Listening for traffic on casperlabs://a96a6c152711416f869da7fe8c2ced61@192.168.1.123:4000.
...
```
--bootstrap casperlabs://a96a6c152711416f869da7fe8c2ced61@192.168.1.123:4000
```

For example:

```
$ ./bin/node run --bootstrap casperlabs://a96a6c152711416f869da7fe8c2ced61@192.168.1.123:4000 -p 4001
...
11:24:10.674 [main] INFO  main - Listening for traffic on casperlabs://5fb06848ea89457e82a9e37565df2d57@192.168.1.123:4001.
