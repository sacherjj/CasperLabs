from casperlabs_client.key_pairs import EthereumKey


def test_generate_ethereum():
    key_pair = EthereumKey.generate()

    assert len(key_pair.private_key) == 24
...
def test_ethereum_round_trip_private_key():
    ...
    first_key = EthereumKey(private_key=private_key)
    private_pem = first_key.private_key_pem  # generated from private_key given
    other_key = EthereumKey(private_key_pem=private_pem)
...
def test_ethereum_round_trip_private_key_pem():
    ...
    other_key = EthereumKey(private_key=private_key)
