...
from casperlabs_client import CasperLabsClient, key_pairs
...
def test_sign_deploy(account_keys_directory, algorithm):
    ...
    signed_by_key_pair = client.sign_deploy(
        key_pair=key_pair, algorithm=algorithm, deploy=deploy
    )
    assert signed_by_pem == signed_by_key_pair
