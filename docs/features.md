...
## Network Launch
### As a CasperLabs SRE I want to launch a network
#### A successful genesis ceremony 

##### test: test/test_genesis_ceremony.py::test_successful_genesis_ceremony 
##### steps:

* `ceremonyMaster` is instantiated with flags `--required-sigs 2 --duration 5min --interval 10sec --bonds-file <holds two nodes validatorA and validatorB`.
...
## Wallets
### As a user, I want to be able to create a wallet so that I can store TODO-COIN-NAME in it
### As a user, I want to be able to add TODO-COIN-NAME to my wallet so that I have available TODO-COIN-NAME to pay for goods/services
### As a user, I want to be able to remove TODO-COIN-NAME from my wallet so that I can pay for goods/services
### Expose purses inside a wallet
### As a wallet application, I want to query a wallet contract (or the blocks) for the history of all TODO-COIN-NAME transfers to/from it
### As a TODO-COIN-NAME holder, I can move some of my TODO-COIN-NAME to the control another user’s public key (or address) via a co-op supplied dApp wallet
### As a recipient of TODO-COIN-NAME (other than Genesis TODO-COIN-NAME), I can use a co-op supplied dApp to view my TODO-COIN-NAME balance
### As a validator, I can move TODO-COIN-NAME to/from the key-pair for one validator node to the key-pair for another validator node or that of the co-op supplied wallet dApp
### As a wallet dApp developer, I want to use Ethereum-style addresses for send transactions to specify the recipient, so that a) I can reuse available Ethereum utility libraries; b) so the QR code is smaller and thus faster to scan than it would be for a full public key; c) it is easier for users to verbally confirm their address versus public key; and d) so CasperLabs is more palatable for the Ethereum community
### As a wallet dApp developer, I want to discover and connect to CasperLabsNodes that support a particular version (release number and hash) and have a minimum number of active connections, so that user risks due to interoperability issues and sybil actors are minimized
...
## Consensus
### As a dApp developer I want to be able to deploy my WASM contract to a validator
...
* call `casperlabs deploy` with invalid contract on `ceremonyMaster`
* assert a error logs on std out
* `casperlabs deploy` exit code should be 1
